...
knitr::opts_chunk$set(echo = FALSE, warning=FALSE, message=FALSE,collapse=T, cache=F, 'styler', tidy.opts=list(strict=T), fig.width = 12,fig.height=10)
```
...
colors <- rep(colors, num_reps)
  ...
  geom_bar(stat = "identity", position="dodge")+
    ...
    theme_bw()+
    theme(axis.text.x = element_text(angle = 90, hjust = 1, size = min(0.1, 5 / 2*length(value_counts))), legend.position = "none", plot.title = element_text(hjust = 0.5))+
    labs(fill="")+scale_y_continuous(expand=c(0,0),limits = c(0,max_y_value))+scale_x_discrete(expand=c(0,0))+ggtitle(plot_title)+
    ylab("Counts")+
    xlab(column)#+geom_text(aes(label = x), angle = 90, hjust = 1, cex = min(1, 10 / length(value_counts)))
}
...
tabsetPanel(
  tabPanel("Norway_historic", make_interactive_barplot(data = Metadata, column = "Serotype", dataset = "Norway_historic")),
  tabPanel("Norway_routine", make_interactive_barplot(data = Metadata, column = "Serotype", dataset = "Norway_routine")),
